...
=======
        ...
        #     self.log_error = LogFile("/home/racecar/distancesPPlog1.csv",["distance"])
    def fake_cb(self, msg):
>>>>>>> 154e90f5704119791bbfe91cb77b4fa08ee7c5d9
        pass
    ...
    def map_to_car_convert(self, result, exp_position, exp_quaternion):
        ...
        x, y, _, _ = np.dot(
            tf.transformations.quaternion_matrix(inverted_quat), homo_coor)
        return x, y
    ...
    def odom_callback(self, msg):
        ...
        """
        self.traj_message = self.traj_message[self.safety_point_index:]
        ...
        try:
            ...
            #closest = np.argmin(
            #    np.abs(np.array(segment_lengths) - self.lookahead))
            if closest < self.safety_point_index:
                closest = self.safety_point_index
        except:
            print("Can't find traj, will try again")
            return
        ...
        if angle_threshold > 0.25:
            self.drive_cmd.drive.speed = self.speed * (2. - angle_threshold)
